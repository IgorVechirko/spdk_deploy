...
echo "Terminating $dev device binary on $node node..."

spdk_path=$(get_dev_node_field $dev $node "spdk_path")

terminate_cmd="$spdk_path/scripts/rpc.py spdk_kill_instance SIGTERM"
...
exe_on_host $host_addr $user $pass "$terminate_cmd"